-- Inventário do jogador (Forja + baús de suprimentos).
-- Migration idempotente: pode ser aplicada em banco novo ou já migrado.
--   - item_category: 'supply_chest' (empilhável)
--                   | 'weapon' | 'helmet' | 'chest' | 'boots' (peças de equipamento = 1 linha cada)
--   - rarity: 'common' | 'rare' | 'epic'
--   - name: nome do equipamento sorteado (NULL para baús)
--   - level: 0..12 (nível de refino; sempre 0 em baús)
--   - quantity: > 0 (baús acumulam a quantidade; equipamentos sempre 1)
--   - equipped: marca a peça equipada no paperdoll (máx. 1 por slot)
...
-- Compatibilidade: caso a tabela já exista de uma versão anterior da feature,
-- garante as colunas de equipamento e remove a constraint antiga de empilhável
-- global (equipamentos não são empilháveis; cada peça é 1 linha distinta).
...
-- Baús criados por versões antigas da feature usavam item_category = 'chest'.
update public.inventory
set item_category = 'supply_chest'
where item_category = 'chest';

-- Regra de forma dos registros.
alter table public.inventory
...
-- Acúmulo de baús de suprimentos: 1 linha por (user, 'supply_chest', rarity).
...
drop index if exists inventory_user_idx;
create index inventory_user_idx on public.inventory (user_id);

-- Row Level Security: usuário autenticado só enxerga/alterar os próprios registros.
...
-- Abre um baú de suprimentos: consome 1 unidade e sorteia/insere uma peça nível 0.
