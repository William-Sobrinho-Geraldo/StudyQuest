-- ═══════════════════════════════════════════════════════════════════
-- Correção: buy_shop_item não deve inserir avatar em inventory.
--
-- O inventory possui a check constraint inventory_item_shape_check
-- (aceita apenas weapon/helmet/chest/boots/supply_chest). A geração
-- do mercado passou a sortear avatares (item_category = 'avatar'), mas
-- a versão antiga de buy_shop_item sempre inseria em inventory,
-- estourando a constraint ao comprar um avatar.
--
-- Regra corrigida:
--   • item_category = 'avatar' → desbloqueia via array_append em
--     profiles.unlocked_avatars (NÃO insere em inventory).
--   • demais categorias        → mantém o fluxo original (INSERT INTO
--     inventory de equipamento).
--   • ambos marcam o slot como comprado e retornam o gold restante.
-- ═══════════════════════════════════════════════════════════════════
