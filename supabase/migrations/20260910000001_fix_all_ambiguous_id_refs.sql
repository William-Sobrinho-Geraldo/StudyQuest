-- Fix completo: qualifica TODAS as referências bare `id` em refine_item e open_inventory_chest.
--
-- O `returns table (id uuid, ...)` cria colunas de saída que competem com
-- QUALQUER referência solta a `id` no corpo PL/pgSQL. Toda query que toca
-- `inventory` ou `profiles` precisa usar o nome da tabela (ou alias) no WHERE.
